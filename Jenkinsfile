pipeline {
  ...
  environment {
    ...
    GITHUB_REPO = "ergon-automation-labs/ergon-notion"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/notion_bot/bin/notion_bot"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyNotion.Release.create()'"
          $RELEASE_BIN eval 'BotArmyNotion.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyNotion.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyNotion.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_notion/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_notion/releases/start_erl.data)
        fi
